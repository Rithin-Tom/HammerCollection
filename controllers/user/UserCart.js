...
const loadCart = async (req, res) => {
  ...
  return res.render("user/userCart", { 
    user, 
    cart: { items: [], totalPrice: 0, totalAmount: 0, totalItems: 0 } 
  });
}

   

   const validItems = cart.items.filter((ele)=>ele.productId.quantity>0)
...
const addToCart = async (req, res) => {
  try {
    ...
    if (exitingItem) {
      if(exitingItem.quantity ==product.quantity)return res .status(401).json({success:false,message:`Out of stock ` })
  ...
  try {
    ...
    if (quantity > 5) {
      return res
        ...
        .json({
          success: false,
          message: "You can only add 5 item in a single product",
        });
    }
    if (quantity > product.quantity) {
      return res
        ...
        .json({
          success: false,
          message: `${product.productName} has only  have ${items.quantity} stocks left`,
        });
    }
...
const deleteItem = async (req, res) => {
  try {
    ...
    if (!cart){
      return res
        .status(404)
        .json({ success: false, message: "cannot find the cart" });
    }
