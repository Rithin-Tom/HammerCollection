    ...
    if (!user) {
      ...
      res
        ...
        .json({
          ...
          redirectUrl: `/verify-otp?email=${email}&purpose=signup`,
        });
    } else {
  ...
  try {
    ...
    if (otp === req.session.userOtp) {
      if (purpose === "signup") {
        ...
        return res.json({
          success: true,
          redirectUrl: `/profile/edit?email=${encodeURIComponent(email)}`,
        });
      }
    } else {
      res
  ...
  try {
    ...
    if (!password || !email) {
      return res
        ...
        .json({
          success: false,
          message: "Missing password or session expired.",
        });
    }
